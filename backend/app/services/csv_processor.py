...
class CsvProcessorService(BaseService):
    def __init__(self, logger=None):
        ...
        self.error_handlers: List[Callable[[Exception, Optional[Any]], None]] = []
    ...
    def add_error_handler(self, handler: Callable[[Exception, Optional[Any]], None]):
        ...
        for strategy in self.validation_strategies:
            try:
                if not strategy(file_path):
                    ...
                    if strategy.__name__ == "schema_validation" and hasattr(strategy, "missing_columns") and strategy.missing_columns:
                        error_detail = f"Missing columns: {strategy.missing_columns}"
                    error = {"file": file_path, "strategy": strategy.__name__, "error": error_detail}
                    self.validation_errors.append(error)
                    self.log_error(f"Validation failed for {file_path} with {strategy.__name__}: {error_detail}")
                    all_passed = False
            except Exception as e:
                error = {"file": file_path, "strategy": strategy.__name__, "error": str(e)}
                self.validation_errors.append(error)
                self.handle_error(e, context={"file_path": file_path, "strategy": strategy.__name__})
                all_passed = False
        return all_passed
    ...
    @staticmethod
    def schema_validation(expected_columns: List[str]) -> Callable[[str], bool]:
        ...
        def validate(file_path: str) -> bool:
            ...
            missing = [col for col in expected_columns_clean if col not in df.columns]
            if missing:
    ...
    @staticmethod
    def business_rule_validation(rule: Callable[[pd.DataFrame], bool]) -> Callable[[str], bool]:
        def validate(file_path: str) -> bool:
    ...
    @staticmethod
    def data_quality_check(check: Callable[[pd.DataFrame], bool]) -> Callable[[str], bool]:
        def validate(file_path: str) -> bool:
            df = pd.read_csv(file_path)
            df.columns = [col.strip().lower() for col in df.columns]
            return check(df)
            ...
            for chunk in pd.read_csv(file_path, chunksize=chunk_size):
                ...
                for step in self.pipeline_steps or self.transformations:
                    try:
                        ...
                        self.handle_error(e, context={"stage": "pipeline", "step": getattr(step, '__name__', str(step))})
    ...
    async def process_async(self, file_path: str, chunk_size: int = 10000) -> int:
        if not self.validate_file(file_path):
            self.log_error(f"File validation failed: {file_path}")
            return 0
        processed_rows = 0
    ...
    def handle_error(self, error: Exception, context: Optional[Any] = None) -> None:
        super().handle_error(error, context)
        for handler in self.error_handlers:
            try:
                handler(error, context)
            except Exception as e:
                self.log_error(f"Error in error handler: {str(e)}", context=context)
