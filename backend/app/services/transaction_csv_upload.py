...
class TransactionCsvUploadService:
    def __init__(self, upload_dir: str = "uploads", logger=None):
        self.upload_dir = upload_dir
        self.logger = logger
        os.makedirs(self.upload_dir, exist_ok=True)
        self.csv_processor = CsvProcessorService(logger=logger)
    ...
    async def cleanup_file(self, file_path: str) -> None:
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                if self.logger:
                    self.logger.info(f"Successfully cleaned up file: {file_path}")
        except Exception as e:
            if self.logger:
    ...
    async def process_and_insert(self, db: AsyncSession, file) -> int:
        ...
        try:
            if not self.csv_processor.validate_file(file_location):
                ...
                missing_cols = None
                for err in errors:
                    if err["strategy"] == "schema_validation" and "Missing columns" in err["error"]:
                        missing_cols = err["error"]
                if missing_cols:
                    raise ValueError(f"CSV schema validation failed: {missing_cols}")
            ...
            def convert_column(series, target_type):
                if target_type is Integer:
                    ...
                    cleaned = cleaned.astype(str).str.strip().str.strip("'").str.strip('"')
                    dt = pd.to_datetime(cleaned, format='%m/%d/%Y %I:%M:%S %p', errors='raise', utc=True)
                    return dt.dt.tz_localize(None)
                elif target_type is String:

                    trimmed = series.astype(str).str.strip().str.strip("'").str.strip('"')
                    cleaned = trimmed.replace(r'^$', None, regex=True)
                    return cleaned.apply(lambda x: str(x) if x is not None else None)
                else:
                    return series
            ...
            for col, target_type in model_type_map.items():
                if col in df.columns:
                    try:
                        ...
                        raise ValueError(f"Failed to convert column '{col}' to {target_type.__name__}: {e}")
